"""
file_processor.py — KITT Universal File Processor
...
def _gemini_client():
    """Cliente Gemini — API nueva (google-genai)."""
    return _genai_new.Client(api_key=_get_api_key())


def _gemini_generate(prompt: str, image=None) -> str:
    """Genera texto con Gemini 2.5 Flash usando la API nueva."""
    ...
    if image is not None:
        ...
        response = client.models.generate_content(
            ...
            contents=[
                ...
                _genai_types.Part.from_text(text=prompt),
            ]
        )
    else:
        response = client.models.generate_content(
