...
# ─────────────────────────────────────────────────────────────────────────
# NOTA DE SEGURIDAD (auditoría 2026):
# Esta versión eliminó por completo la generación y ejecución de código
# Python arbitrario (exec()) para tareas de escritorio en lenguaje natural.
# El enfoque anterior le pedía a Gemini que escribiera código Python y lo
# ejecutaba con exec() dentro de un "sandbox" de builtins restringidos.
# Ese sandbox era inseguro: exponía getattr/hasattr (la vía clásica de
# escape de sandboxes en Python) y, en Windows, acceso directo a ctypes.
# Cualquier texto que el modelo generara —accidental o inducido por una
# instrucción maliciosa— podía terminar ejecutando código con los permisos
# del usuario.
#
# Ahora, cualquier instrucción en lenguaje natural para el escritorio se
# clasifica con Gemini hacia una de las acciones seguras ya existentes
# (wallpaper, organize, clean, list, stats). Si no encaja en ninguna,
# se informa al usuario en vez de intentar "inventar" código para ello.
# No se ejecuta nunca código generado por el modelo.
# ─────────────────────────────────────────────────────────────────────────

_SAFE_DESKTOP_INTENTS = {
    "wallpaper", "wallpaper_url", "current_wallpaper",
    "organize", "clean", "list", "stats", "none",
}


def _classify_desktop_intent(task: str) -> str:
    """
    Usa Gemini SOLO para clasificar una instrucción en lenguaje natural
    hacia una de las acciones seguras predefinidas. Nunca genera ni
    ejecuta código.
    """
    try:
        from core.gemini_compat import get_model
        model = get_model(_get_api_key(), "gemini-2.5-flash-lite")

        prompt = f"""Classify this desktop-related request into EXACTLY ONE of these labels:

- wallpaper        (user wants to set a wallpaper from a local file path)
- wallpaper_url     (user wants to set a wallpaper from an image URL)
- current_wallpaper (user wants to know the current wallpaper)
- organize          (user wants desktop files organized into folders)
- clean             (user wants desktop files archived/cleaned up)
- list              (user wants to see what's on the desktop)
- stats             (user wants desktop file count / size stats)
- none              (request doesn't match any of the above)
...
Request: "{task}" """
        ...
        label = response.text.strip().lower().split()[0] if response.text.strip() else "none"
        return label if label in _SAFE_DESKTOP_INTENTS else "none"
    except Exception as e:
        print(f"[Desktop] Intent classification failed: {e}")
        return "none"
...
def desktop_control(
    ...
    try:
        if action == "wallpaper":
            ...
            if player:
                player.write_log("[Desktop] Identifying the requested action...")

            intent = _classify_desktop_intent(actual_task)
            if intent == "none":
                return (
                    "I can't safely perform that specific desktop task. "
                    "I can set a wallpaper, organize/clean the desktop, "
                    "list its contents, or give you stats — try rephrasing "
                    "your request around one of those."
                )
            # Delegar a la acción segura ya validada, nunca a código generado.
            return desktop_control(
                parameters={**params, "action": intent},
