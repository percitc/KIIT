...
def _get_model(model_name: str):
    ...
    return get_model(_get_api_key(), model_name)
...
def _open_vscode(project_dir: Path) -> bool:
    ...
    for cmd in vscode_candidates:
        try:
            # Sin shell=True: evita que el nombre del proyecto (que puede
            # venir de una descripción libre del usuario) sea interpretado
            # por una shell si contiene caracteres especiales.
            # Los .cmd/.bat de Windows necesitan pasar por cmd.exe /c para
            # ejecutarse, pero se hace con argumentos separados (no una
            # cadena concatenada), así no se reintroduce la inyección.
            if cmd.lower().endswith((".cmd", ".bat")):
                argv = ["cmd", "/c", cmd, str(project_dir)]
            else:
...
def _build_project(
    ...
    proj_name    = project_name or plan.get("project_name", "kitt_project")
